...
The docker image will run matrix-pinbot with a SQLite database and
...
* Choose whether you want to use SQLite or Postgres as your database backend.
  ...
  ```
  database: "postgres://username:password@postgres/matrix-pinbot?sslmode=disable"
  ```
...
```
docker-compose up matrix-pinbot
```
...
```
docker-compose pull matrix-pinbot
```
...
[matrix-pinbot.service](matrix-pinbot.service). The service uses
`docker-compose` to start and stop the bot.

Copy the file to `/etc/systemd/system/matrix-pinbot.service` and edit to
match your setup. You can then start the bot with:

```
systemctl start matrix-pinbot
```

and stop it with:

```
systemctl stop matrix-pinbot
```

To run the bot on system startup:

```
systemctl enable matrix-pinbot
```
...
```
docker build -t somebody/matrix-pinbot:latest -f docker/Dockerfile .
```
